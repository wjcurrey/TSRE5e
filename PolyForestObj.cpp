...
PolyForestObj::PolyForestObj(const PolyForestObj& o) : WorldObj(o) {
...
WorldObj* PolyForestObj::clone(){
    return new PolyForestObj(*this);
}
...
void PolyForestObj::LoadPolyForestList(){
    ...
    while (!((sh = ParserX::NextTokenInside(data).toLower()) == "")) {
        qDebug() << sh;
        if (sh == ("polyForest")) {
            PolyForestObj::polyForestList.push_back(PolyForestObj::PolyForestList());
            PolyForestObj::polyForestList.back().name = ParserX::GetString(data);
            PolyForestObj::polyForestList.back().texture = ParserX::GetString(data);
            PolyForestObj::polyForestList.back().treeSizeX = ParserX::GetNumber(data);
            PolyForestObj::polyForestList.back().treeSizeZ = ParserX::GetNumber(data);
            PolyForestObj::polyForestList.back().scaleRangeX = ParserX::GetNumber(data);
            PolyForestObj::polyForestList.back().scaleRangeZ = ParserX::GetNumber(data);
...
int PolyForestObj::GetListIdByTexture(QString texture){
    for(int i = 0; i < polyForestList.size(); i++){
        if(polyForestList[i].texture.toLower() == texture.toLower())
            return i;
    }
    return 0;
}

void PolyForestObj::load(int x, int y) {
    this->x = x;
    this->y = y;
...
void PolyForestObj::set(QString sh, QString val){
    if (sh == ("ref_filename")) {
        treeTexture = val;
        return;
    }
...
void PolyForestObj::set(QString sh, long long int val) {
    if (sh == ("ref_value")) {
        qDebug() << val;
        int val2        = val & 0xFFFF;
        ...
        treeTexture = PolyForestObj::polyForestList[val2].texture;
        scaleRangeX = PolyForestObj::polyForestList[val2].scaleRangeX;
        scaleRangeZ = PolyForestObj::polyForestList[val2].scaleRangeZ;
        areaX = 100;
        if(tareax != 0)
            areaX = tareax;
        areaZ = 100;
        if(tareaz != 0)
            areaZ = tareaz;
        treeSizeX = PolyForestObj::polyForestList[val2].treeSizeX;
        treeSizeZ = PolyForestObj::polyForestList[val2].treeSizeZ;
        population = 10;
        if(tpopulation != 0)
...
void PolyForestObj::set(QString sh, float val) {
    if (sh == ("areaX")) {
        areaX = val;
        return;
    }
...
void PolyForestObj::set(int sh, FileBuffer* data) {
...
void PolyForestObj::set(QString sh, FileBuffer* data) {
    if (sh == ("treetexture")) {
...
Ref::RefItem* PolyForestObj::getRefInfo(){
...
void PolyForestObj::translate(float px, float py, float pz){
    this->position[0]+=px;
    //this->position[1]+=py;
    this->position[2]+=pz;
    setModified();
...
void PolyForestObj::rotate(float x, float y, float z){
...
void PolyForestObj::resize(float x, float y, float z){
    if(x == 0 && y == 0) return;
    this->areaX += x;
    this->areaZ += y;
    setModified();
...
bool PolyForestObj::getBoxPoints(QVector<float>& points){
    if (!loaded) 
        return false;
...
int PolyForestObj::getDefaultDetailLevel(){
    return -11;
}

void PolyForestObj::save(QTextStream* out){
    if (!loaded) return;
...
bool PolyForestObj::allowNew(){
    return true;
}

void PolyForestObj::deleteVBO(){
